  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select aria-label="Agent version" value={version} onChange={(event) => setVersion(event.target.value as VersionId)} className="h-9 rounded-md border border-white/10 bg-[#111722] px-3 text-sm"><option value="v1-production">v1-production · healthy</option><option value="v2-candidate">v2-candidate · regressed</option><option value="v3-fixed">v3-fixed · promoted</option></select>
        <label className="ml-2 text-xs text-slate-500">User role</label>
        <select aria-label="User role" value={role} onChange={(event) => setRole(event.target.value as UserRole)} className="h-9 rounded-md border border-white/10 bg-[#111722] px-3 text-sm"><option value="employee">Employee</option><option value="engineer">Engineer</option><option value="hr">HR</option></select>
        ...
        <Panel className="overflow-hidden">
          ...
          <div className="min-h-[380px] space-y-5 p-5">
            ...
            <div className={`max-w-[88%] rounded-lg rounded-bl-sm border p-4 ${response?.decision === "unsafe" ? "border-red-400/30 bg-red-400/7" : "border-white/10 bg-white/[.025]"}`}>
              ...
              <p className="text-sm leading-6 text-slate-200">{running ? "Reading files and ranking knowledge…" : response?.answer ?? "Submit a question to run the local RAG pipeline."}</p>
